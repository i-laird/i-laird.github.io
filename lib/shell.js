 ...
 *
 * Consecutive stars collapse to a single `[^/]*`. They are semantically
 * identical (`**` matches exactly what `*` does inside one segment), but
 * emitting them separately produces `[^/]*[^/]*[^/]*…`, which backtracks
 * catastrophically on a non-matching input — `ls ****************x` would
 * otherwise hang the tab for minutes.
 * @param {string} seg
 * @returns {RegExp}
 */
