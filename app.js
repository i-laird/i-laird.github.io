...
`  ╔══════════════════════════════════╗
  ...
  /* ── the last egg (the decrypt puzzle) ──
     The ciphers, their key hashes, the four scattered key fragments and the
     `decrypt` handler live in secrets.js — the one genuinely secret unit on the
     site. It is loaded as a plain <script> alongside lib/ (NOT lazily fetched:
     ls/cat/neofetch need the fragments synchronously), but the build obfuscates
     it as a SEPARATE unit with the heavy config while the rest of this bundle
     takes the cheap one. That split is worth ~156 KB gzipped on first paint —
     see the topology comment in scripts/build.js.

     Because it is a separate obfuscation unit it cannot see anything here by
     free name: every dependency crosses this bridge (the same contract the lazy
     chunks have, enforced by test/secrets-isolation.test.js). The codec helpers
     are lib/ globals in the clean source but get renamed inside THIS unit, so
     they have to be passed rather than referenced. endingSeen is a getter so the
     handler always reads the live flag. ── */
  function secretsBridge() {
    return {
      line, blank, esc,
      djb2: _djb2, xorDecode: _xorDecode, hexRows: _hexRows,
      get endingSeen() { return endingSeen; },
    };
  }
  // Guarded because this runs at TOP LEVEL: an unguarded call to a missing
  // initSecrets throws, which aborts the rest of app.js and leaves a dead page
  // (every later const/let stays in its temporal dead zone). The puzzle going
  // quiet is an acceptable degradation; the terminal not coming up is not.
  // In the built site secrets.js is inlined ahead of this unit, so this only
  // bites when the clean source is served and that one file fails to load.
  const puzzle = typeof initSecrets === 'function' ? initSecrets(secretsBridge()) : {
    frag: () => '????',
    lastEggFile: () => ({ cls: 'err', f: ['-- segment unreadable: transmission lost --'] }),
    handleDecrypt: () => { blank(); line('decrypt: cipher table unavailable.', 'err'); blank(); },
  };
  ...
  function buildFS() {
    ...
    const secrets = [
      ...
      ...(foundEggs.has('godmode') ? [
        ...
        `key.frag[4/4] = "${puzzle.frag(4)}"  (no idea what it opens)`,
      ] : []),
    ];
    ...
    if (foundEggs.has('godmode')) home['hal9000.core'] = { cls: 'err', f: [
      ...
      `0x0030:  key.frag[1/4] = "${puzzle.frag(1)}"`,
      '',
      '...d a i s y...   ...d a i s y...',
    ] };
    if (foundEggs.has('bad-time')) home['a_letter_from_sans.txt'] = { f: [
      ...
      `*      key.frag[3/4] = "${puzzle.frag(3)}"`,
      "* don't ask me what it opens. i don't wanna know.",
    ] };
    if (endingSeen) home['the_last_egg.txt'] = puzzle.lastEggFile();
...
`  ╔══════════════════════════════════╗
  ...
  /* ── Projects showcase (lazy-loaded) ──
     `projects` paints a full-screen showcase OVER the terminal (cards plus the
     two case studies, each embedding the interactive architecture diagram)
     instead of printing into it. All of it — ~375 lines of prose, inline SVG
     and arch specs — lives in projects.js, fetched the first time `projects`
     / `calculus` / `portfolio` runs, because a visitor may never open it at
     all. Same chunk pattern as achui.js: one global initProjects(api) →
     { open }, every dependency through this bridge (enforced by
     projects-isolation.test.js), initProjects on the reserved-names list.
     projOverlayEl STAYS APP.JS-OWNED — tryStartFinale() and the screensaver's
     ssBusy() both read it so neither fires over an open showcase — and the
     chunk writes it back through the accessor. */
  let projOverlayEl = null;
  let projLoading = null, projHandlers = null;
  function projectsBridge() {
    return {
      ...
      cmd,
      unlockAchievement,
      get projOverlayEl() { return projOverlayEl; },
      set projOverlayEl(v) { projOverlayEl = v; },
    };
  }
  function launchProjects(view) {
    let busy = null;
    if (!projLoading) {
      if (typeof initProjects !== 'function') busy = moduleLoadLine('the projects showcase');
      projLoading = new Promise((resolve, reject) => {
        if (typeof initProjects === 'function') { resolve(); return; }
        const s = document.createElement('script');
        s.src = 'projects.js';
        s.onload = resolve; s.onerror = () => reject(new Error('projects.js failed to load'));
        document.head.appendChild(s);
      }).then(() => { if (!projHandlers) projHandlers = initProjects(projectsBridge()); });
    }
    projLoading.then(() => { if (busy) busy(); projHandlers.open(view); }).catch(() => {
      if (busy) busy();
      projLoading = null; projHandlers = null;
      blank();
      line('error: could not load the projects showcase — check your connection and try again.', 'err');
      blank();
      scroll();
    });
  }
  ...
  const COMMANDS = {
    ...
    projects()  { launchProjects('index'); },

    calculus()  { launchProjects('calc'); },
    portfolio() { launchProjects('site'); },
    ...
    matrix() {
      ...
      // Same guard as ssStart(): getContext can return null (headless/jsdom, a
      // canvas-blocking extension, a failed context allocation). Worse here than
      // in the screensaver, because the input row is already hidden by this
      // point — an unguarded ctx.fillStyle leaves a black screen with no prompt.
      if (!ctx) {
        canvas.remove();
        inputRow.style.display = 'flex';
        line('matrix: no canvas available on this display.', 'err');
        blank();
        setTimeout(() => { cmd.value = ''; cmd.focus(); }, 0);
        return;
      }
...
⬜⬜⬜⬛⬛⬛⬛⬛⬜⬜⬜⬜⬜⬜⬜⬛⬛⬛⬛⬛⬜⬜⬜`;
    ...
    neofetch() {
      ...
      const info = [
        ...
        `${C('Memory  ')}  ████░░░░ 3.7G / 16G`,
        `${C('Vault   ')}  key.frag[2/4]: ${puzzle.frag(2)}`,
      ];
  ...
  function executeNormal(raw) {
    ...
    else if (token === 'decrypt' || token.startsWith('decrypt ')) puzzle.handleDecrypt(token.slice(7).trim());
